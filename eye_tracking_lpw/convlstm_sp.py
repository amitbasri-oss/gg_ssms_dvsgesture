...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        ...
        th = 0.005
        # delta = h_cur - h_pre
        # threshold = torch.tensor(0.05).cuda()
        ...
        #     sparse_rate_delta = (h_cur.numel() - torch.count_nonzero(h_cur).float()) / h_cur.numel()
        #     file_path = os.path.join(log_dir, f"sparse_rate_th_{th:.5f}.txt")
        ...
        # combined_conv = torch.relu(combined_conv)
        ...
        delta = h_next - h_cur
        threshold = torch.tensor(th).cuda()
        h_next_delta = torch.where(delta < threshold, torch.tensor(0.0).cuda(), delta)

        return h_next_delta, c_next, h_cur
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                h, c, h_prev = self.cell_list[layer_idx](
