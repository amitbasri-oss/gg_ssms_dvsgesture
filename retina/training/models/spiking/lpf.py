...
class LPFOnline(nn.Module):
    def __init__(
        ...
        self.scale_factor = nn.Parameter(
            torch.tensor(initial_scale), requires_grad=train_scale
        )
    ...
    def plot(self, kernel, step):
        ...
        plt.xticks(
            np.arange(self.kernel_size, -1, -1),
            np.arange(self.kernel_size, -1, -1).astype(int),
        )
    ...
    def set_custom_kernel(self):
        ...
        syn_kernel = (
            torch.exp(-torch.arange(self.kernel_size) / self.tau_syn)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        mem_kernel = (
            torch.exp(-torch.arange(self.kernel_size) / self.tau_mem)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        kernel = torch.nn.functional.conv1d(syn_kernel.flip(-1), mem_kernel.flip(-1))[
            ..., :-1
        ]
        return kernel
    ...
    def forward(self, x, padding_mode="past"):
        ...
        syn_kernel = (
            torch.exp(-torch.arange(self.kernel_size).to(x.device) / self.tau_syn)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        mem_kernel = (
            torch.exp(-torch.arange(self.kernel_size).to(x.device) / self.tau_mem)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        padding = torch.zeros_like(syn_kernel)
