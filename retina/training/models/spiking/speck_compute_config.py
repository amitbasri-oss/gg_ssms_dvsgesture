...
for layer in layers:
    if layer[0] == "Conv":
        ...
        neuron_memory = calculate_neuron_memory(
            f, c_x, c_y, k_x, k_y, s_x, s_y, p_x, p_y
        )
...
for i, (name, kernel_memory, neuron_memory) in enumerate(results):
    ...
    for key in core_memory.keys():
        if core_memory[key][0] >= float(kernel_memory_str) and core_memory[key][
            1
        ] >= float(neuron_memory_str):
            compatible_cores = compatible_cores + f", {key}"

    print(
        f"{i+1} & {name} & {kernel_memory_str} KiB & {neuron_memory_str} KiB & {compatible_cores} \\\\"
    )
print("\\end{tabular}")
