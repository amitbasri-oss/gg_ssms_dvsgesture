...
class LSQActivationQuantizer(nn.Module):
    ...
    def forward(self, activation):
        ...
        if self.init_state == 0:
            ...
            self.s.data = (
                torch.mean(torch.abs(activation.detach())) * 2 / (math.sqrt(self.Qp))
            )
            self.init_state += 1
        elif self.init_state < self.batch_init:
            self.s.data = 0.9 * self.s.data + 0.1 * torch.mean(
    ...
    def forward(self, weight):
        if self.init_state == 0:
            ...
            if self.per_channel:
                ...
                self.s.data = (
                    torch.mean(torch.abs(weight_tmp), dim=1) * 2 / (math.sqrt(self.Qp))
                )
            else:
                self.s.data = (
                    torch.mean(torch.abs(weight.detach())) * 2 / (math.sqrt(self.Qp))
                )
            self.init_state += 1
        elif self.init_state < self.batch_init:
            if self.per_channel:
