...
data_module = EyeTrackingDataModule(
    dataset_params=dataset_params, training_params=training_params, num_workers=16
)
...
def representative_dataset():
    for input_data, _ in train_dataloader:
        input_data = (
            input_data.detach().cpu().numpy().astype(np.float32)
        )  # Convert PyTorch tensor to NumPy
        print(
            f"Step {step}: Input shape = {input_data.shape}"
...
converter = tf.lite.TFLiteConverter.from_saved_model(
    os.path.join(path_to_run, "models/model_tf")
)
