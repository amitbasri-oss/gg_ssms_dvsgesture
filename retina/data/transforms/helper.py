...
def get_transforms(
    dataset_params, training_params, augmentations: bool = False
) -> Tuple[Compose, FromPupilCenterToBoundingBox]:
    """
    ...
    target_transforms = FromPupilCenterToBoundingBox(
        yolo_loss=training_params["arch_name"][:6] == "retina",
    ...
    if dataset_params["dataset_name"] == "ini-30" and (
        dataset_params["img_width"] != 640 or dataset_params["img_height"] != 480
    ):
    ...
    input_transforms.append(
        ToFrame(sensor_size=sensor_size, n_event_bins=dataset_params["num_bins"])
    )
