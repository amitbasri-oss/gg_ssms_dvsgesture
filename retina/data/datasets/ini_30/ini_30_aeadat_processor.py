    ...
    def __init__(self, path, filter_noise):
        ...
        if filter_noise:
            self.filter_chain.addFilter(
                dv.RefractoryPeriodFilter(
                    self.recording.getEventResolution(),
                    refractoryPeriod=datetime.timedelta(microseconds=2000),
                )
            )
            self.filter_chain.addFilter(
                dv.noise.BackgroundActivityNoiseFilter(
                    self.recording.getEventResolution()
                )
            )
...
class AedatProcessorLinear(AedatProcessorBase):
    ...
    def __init__(
        ...
        self.accumulator = dv.Accumulator(
            self.recording.getEventResolution(),
