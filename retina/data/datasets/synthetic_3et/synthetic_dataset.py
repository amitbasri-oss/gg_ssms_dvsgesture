...
class SyntheticDataset(Dataset):
    def __init__(
        self, name, training_params, dataset_params, input_transforms, target_transforms
    ):
        ...
        self.folder = sorted(
            [os.path.join(data_dir, "data_ts_pro", name, f + ".h5") for f in filenames]
        )
        target_dir = sorted(
            [os.path.join(data_dir, "label", f + ".txt") for f in filenames]
        )
        self.target = concatenate_files_targets(
            target_dir, num_bins, stride, chunk_size
        )
        ...
        with tables.open_file(file_path, "r") as file:
            ...
            for i in range(len(sample)):
                ...
                sample_resize.append(
                    cv2.resize(
                        sample[i, 0], (int(self.img_width), int(self.img_height))
                    )
                )
