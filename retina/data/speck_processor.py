...
def events_to_label(events: List, shape: Tuple) -> torch.Tensor:
    """
...
def label_to_bbox(predictions: torch.tensor) -> torch.Tensor:
    """dt
    ...
    point_1 = (
        predictions[..., 2:6][..., :2]
        + (predictions[..., 2:6][..., 2:] - predictions[..., 2:6][..., :2]) / 2
    )
    point_2 = (
        predictions[..., 7:11][..., :2]
        + (predictions[..., 7:11][..., 2:] - predictions[..., 7:11][..., :2]) / 2
    )
    ...
    for pred in predictions:
        ...
        if conf_scores_1[best_bbox_index_1] >= conf_scores_2[best_bbox_index_2]:
            bbox_array.append(
                reshaped_predictions[best_bbox_index_1][2:6].clip(0, 1) * 64
            )
            conf_array.append(conf_scores_1[best_bbox_index_1])
        else:
            bbox_array.append(
                reshaped_predictions[best_bbox_index_2][7:11].clip(0, 1) * 64
            )
