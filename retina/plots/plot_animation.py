...
def plot_animation_spikes(events, spikes):
    ...
    def animate(frame_idx):
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                rect[f"{i}_{j}"].set_facecolor(
                    color=(0, (spk[i][j][0].item() / max_possible_value), 0, 0.5)
                )

        return [im, *rect.values()]

    anim = animation.FuncAnimation(
        fig, animate, frames=num_bins, interval=100, blit=True
    )
    plt.close()
...
def plot_animation_points(
    events, target_points, pred_points, include_point1=True, include_point2=True
):
    """ """
    num_bins, channels, height, width = events.shape
    if channels == 2:
