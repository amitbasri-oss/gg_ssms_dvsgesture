...
def launch_fire(
    ...
    dataloader = get_dataloader(
        data_dir, dataset_params=dataset_params, batch_size=batch_size, idxs=indexes
    )
    ...
    for i, (frame, data, name, labels, label_ts) in enumerate(
        tqdm(dataloader, desc="Iteration Loop")
    ):
        ...
        if event_heatmap:
            ...
            for z in range(len(coord)):
                evs_count[coord[z][0], coord[z][1]] += 1
            evs_count = torch.rot90(
                torch.tensor(evs_count.copy()), k=2, dims=(0, 1)
            )  # // 2
