...
rec_list = [
    "2022-04-12_adam_01",
    "2022-04-22_elisa_01",
    "2022-04-26_chenghan_01_L",
    "2022-05-17_andreas_01_R",
    "2022-05-17_l-wolfgang_01_L",
    "2022-05-17_pierre_01_R",
    "2022-04-12_andreas_01",
    "2022-04-22_f-adam_01",
    "2022-04-26_ferdinando_01_L",
    "2022-05-17_giovanni_01_L",
    "2022-05-17_l-wolfgang_01_R",
    "2022-05-17_rokas_01_L",
    "2022-04-12_luca_01",
    "2022-04-22_karol_01",
    "2022-04-26_martina_01_L",
    "2022-05-17_giovanni_01_R",
    "2022-05-17_marwan_01_L",
    "2022-05-17_rokas_01_R",
    "2022-04-12_pierre_01",
    "2022-04-22_marwan_01",
    "2022-04-26_wisam_01_L",
    "2022-05-17_l-adam_01_L",
    "2022-05-17_marwan_01_R",
    "2022-05-17_vincenzo_01_L",
    "2022-04-12_wolfgang_01",
    "2022-04-26_arnaud_01_L",
    "2022-05-17_andreas_01_L",
    "2022-05-17_l-adam_01_R",
    "2022-05-17_pierre_01_L",
    "2022-05-17_vincenzo_01_R",
]
...
def plot_animation_boxes(events, target_bbox):
    ...
    anim = animation.FuncAnimation(
        fig, animate, frames=num_bins, interval=16, blit=True
    )
    plt.close()
...
for rec_name in rec_list:
    ...
    print(
        ...
        (tab.timestamp.iloc[-1] - tab.timestamp.iloc[0]) / len(tab) / 1000,
    )

    # get intermediary labels based on num of bins
    fixed_timestamps = np.linspace(
        tab.timestamp.iloc[0], tab.timestamp.iloc[-1], timestamps
    )
    print(
        "Current Label Frequency",
        (tab.timestamp.iloc[-1] - tab.timestamp.iloc[0]) / timestamps / 1000,
    )
    ...
    for fixed_tmp in fixed_timestamps:
        ...
        if idx == 0:
            ...
            x_axis.append(
                int(
                    tab.iloc[idx - 1]["center_x"] * weight0
                    + tab.iloc[idx]["center_x"] * weight1
                )
            )
            y_axis.append(
                int(
                    tab.iloc[idx - 1]["center_y"] * weight0
                    + tab.iloc[idx]["center_y"] * weight1
                )
            )
    ...
    for i in tqdm(range(timestamps)):
        ...
        valid_idx = (
            (evs_timestamp <= label_ts)
            & (evs_timestamp >= prev_ts)
            & (evs_coord[:, 0] < 576)
            & (evs_coord[:, 0] >= 64)
        )
        ...
        np.add.at(
            video_array[i, 0],
            (valid_xy[valid_pol == 0, 1], valid_xy[valid_pol == 0, 0]),
            1,
        )
        np.add.at(
            video_array[i, 1],
            (valid_xy[valid_pol == 1, 1], valid_xy[valid_pol == 1, 0]),
            1,
        )
    ...
    # make video
    anim = plot_animation_boxes(
        torch.rot90(torch.tensor(video_array), k=2, dims=(2, 3)), label_array
    )
    print("Saving..")
