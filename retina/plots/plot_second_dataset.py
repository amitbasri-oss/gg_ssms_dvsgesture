...
def read_aerdat(filepath):
    ...
    packet_size = struct.calcsize(
        "=" + packet_format
    )  # 16 + 16 + 8 + 32 bits => 2 + 2 + 1 + 4 bytes => 9 bytes
...
def get_temporal_data(subject, eye):
    ...
    preprocessed_file = os.path.join(
        data_dir, f"user{subject}", str(eye), "preprocessed_data.pkl"
    )
    ...
    event_file = read_aerdat(
        os.path.join(data_dir, "user" + str(subject), str(eye), "events.aerdat")
    )
    generator = (event_file[((i + 1) * 4 - 4)] for i in range(len(event_file) // 4))

    timestamps, accumulated_counts = np.unique(
        [t for t in generator], return_counts=True
    )
...
def get_spatial_data(subject, eye):
    print(f"User {subject}, Eye {eye}")
    preprocessed_file = os.path.join(
        data_dir, f"user{subject}", str(eye), "spatial_data.pkl"
    )
    ...
    generator = (
        ...
        for i in range(len(event_file) // 4)
    )
